...
public class BossPhase2TeleportStartState : BossPhase2State
{
    ...
    public override void UpdateState()
    {
        ...
        if (time > delayTime)
        {
            ...
            if (controller.GetPlayerDistance() <= fsmInfo.MeleeAttackDistance)
            {
                controller.TransactionToState(BossPhase2FSMController.STATE.TELEPORTATTACK);
                return;
            }

            controller.TransactionToState(BossPhase2FSMController.STATE.TELEPORTEND);
            return;
        }
    }
